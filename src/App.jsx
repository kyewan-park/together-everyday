...
const ALL_MEMBERS = [
  ...
  { id: 3, name: "다정",  role: "디자이너", avatar: "🌿", canHelp: true  },
  { id: 4, name: "지유",  role: "디자이너", avatar: "🎨", canHelp: true  },
  { id: 5, name: "계완",  role: "점장",   avatar: "📋", canHelp: false },
];
...
export default function App() {
  ...
  const [zoneDone, setZoneDone] = useState({}); // {z1: ['샴푸대 청소', ...]}
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
          ...
          {/* 내 프로필 선택 */}
          <select value={myId} onChange={e => { setMyId(Number(e.target.value)); setDone([]); }}
            style={{
              background: "rgba(255,255,255,0.15)", color: "#fff", border: "none",
              borderRadius: 12, padding: "6px 10px", fontSize: 12, outline: "none",
            }}>
            {ALL_MEMBERS.map(m => (
              <option key={m.id} value={m.id} style={{ color: C.ink }}>
                {m.avatar} {m.name} {m.id === 5 ? "(점장)" : ""}
              </option>
            ))}
          </select>
        </div>
            ...
            {isJangnim ? (
              <div>
                <div className="card" style={{
                  ...
                  <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 6 }}>계완 점장님 모드</div>
        ...
        {tab === "clean" && (
          <div>
            ...
            {CLEAN_ZONES.map((zone, i) => {
              ...
              const doneList = zoneDone[zone.id] || [];
          ...
          <div>
            ...
            <div className="card" style={{
              ...
              <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.8 }}>
                점장님은 <b style={{ color: C.ink }}>매장 전체 관리</b>에 집중해요.<br/>
                시술 서브 요청은 <b style={{ color: C.sageDeep }}>서희·민경·다정·지유</b>님께 부탁해요 🙏<br/>
                <br/>
                점장님이 편하게 매장 관리에만 집중할 수 있도록<br/>
                디자이너분들이 서로 먼저 챙겨주세요 💛
              </div>
            </div>
