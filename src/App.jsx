            ...
            {salonSub === "sales" && (() => {
              ...
              return (
                <div>
                  ...
                  {/* 매출 입력 (본인만) */}
                  {!isJangnim && (() => {
                    const todayDateStr = `${thisMK}-${String(todayDay).padStart(2, "0")}`;
                    const todayRec = sales.find(s => s.memberId === myId && s.date === todayDateStr);
                    const inputStyle = {
                      width: "100%", border: `1px solid ${C.line}`, borderRadius: 10,
                    ...
                    <div className="card" style={{
                      ...
                      <div style={{ fontSize: 12, fontWeight: 700, color: C.ink, marginBottom: 12 }}>
                        오늘 매출 입력 ({mo + 1}월 {todayDay}일)
                        {todayRec && <span style={{ fontSize: 10, color: C.sageDeep, marginLeft: 6 }}>✓ 입력됨</span>}
                      </div>
                      ...
                      <button onClick={() => {
                        ...
                        setSales(p => [
                          ...p.filter(s => !(s.memberId === myId && s.date === todayDateStr)),
                          { id: Date.now(), memberId: myId, date: todayDateStr,
                      ...
                      <div style={{ fontSize: 10, color: C.muted, marginTop: 8, lineHeight: 1.5 }}>
                        비워두면 0으로 저장돼요 · 다시 저장하면 오늘 값을 덮어써요<br/>
                        선불권은 판매한 <b>개수</b>를 입력하세요
                      </div>
                    </div>
                    );
                  })()}
