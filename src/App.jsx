              ...
              return (
                <div>
                  ...
                  {isJangnim && (
                    <div className="card" style={{
                      ...
                      <div style={{ fontSize: 12, fontWeight: 700, color: C.ink, marginBottom: 12 }}>
                        👥 팀 매출 현황 ({mo + 1}월 {todayDay}일 기준)
                      </div>
                      {targetMembers.map(m => {
                        const tCum = cumUpto(m.id, thisMK, todayDay);
                        const pCum = cumUpto(m.id, prevMK, Math.min(todayDay, daysInPrevMonth));
                        const mAhead = tCum >= pCum;
                        const mStore = monthSumOf(m.id, thisMK, "store");
                        const mPrepaid = monthSumOf(m.id, thisMK, "prepaid");
                        return (
                          <div key={m.id} style={{
                            ...
                            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                              ...
                              <span style={{ fontSize: 13, fontWeight: 700, color: C.ink, flex: 1 }}>{m.name}</span>
                              <div style={{ textAlign: "right" }}>
                                <div style={{ fontSize: 13, fontWeight: 800, color: C.ink }}>{fmt(tCum)}원</div>
                                <div style={{ fontSize: 9, color: mAhead ? C.sageDeep : C.roseDeep, fontWeight: 700 }}>
                                  실매출 {mAhead ? "▲" : "▼"} {fmt(Math.abs(tCum - pCum))}
                                </div>
                              </div>
                            </div>
                            <div style={{ display: "flex", gap: 6 }}>
                  ...
                  <div style={{ fontSize: 10, color: C.muted, textAlign: "center", lineHeight: 1.6, padding: "0 10px" }}>
                    💡 매출은 {isJangnim ? "대표님과 본인만" : "본인과 대표님만"} 볼 수 있어요<br/>
                    매일 입력하면 저번 달의 나와 경주하는 재미가 생겨요 🏃
                  </div>
                </div>
              );
            })()}
