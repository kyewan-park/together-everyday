...
// ── 재고 분류 ──
const INV_CATS = ["펌제", "염모제", "탈색·산화", "트리트먼트", "두피케어", "매장 소모품", "스타일링", "판매용"];
...
export default function App() {
  ...
  const [inventory, setInventory] = useState([
    { id: 1,  category: "펌제",        name: "일반펌 1제",          par: 4,  current: 3 },
    { id: 2,  category: "펌제",        name: "일반펌 2제(중화제)",   par: 4,  current: 2 },
    { id: 3,  category: "펌제",        name: "볼륨매직 1제",         par: 3,  current: 2 },
    { id: 4,  category: "펌제",        name: "볼륨매직 2제",         par: 3,  current: 3 },
    { id: 5,  category: "염모제",      name: "새치커버 자연갈색",     par: 6,  current: 5 },
    { id: 6,  category: "염모제",      name: "패션컬러 애쉬",        par: 6,  current: 2 },
    { id: 7,  category: "염모제",      name: "패션컬러 브라운",       par: 6,  current: 4 },
    { id: 8,  category: "탈색·산화",   name: "탈색 파우더",          par: 3,  current: 2 },
    { id: 9,  category: "탈색·산화",   name: "산화제 6%",            par: 4,  current: 1 },
    { id: 10, category: "탈색·산화",   name: "산화제 9%",            par: 3,  current: 2 },
    { id: 11, category: "트리트먼트",  name: "시술용 클리닉 앰플",     par: 30, current: 22 },
    { id: 12, category: "트리트먼트",  name: "단백질 트리트먼트",      par: 3,  current: 1 },
    { id: 13, category: "두피케어",    name: "스케일링제",           par: 2,  current: 1 },
    { id: 14, category: "매장 소모품", name: "백바용 샴푸",          par: 4,  current: 3 },
    { id: 15, category: "매장 소모품", name: "백바용 린스",          par: 3,  current: 1 },
    { id: 16, category: "매장 소모품", name: "펌지",                par: 4,  current: 2 },
    { id: 17, category: "매장 소모품", name: "염색용 호일",          par: 6,  current: 5 },
    { id: 18, category: "매장 소모품", name: "일회용 비닐장갑",       par: 8,  current: 6 },
    { id: 19, category: "매장 소모품", name: "비닐캡",              par: 5,  current: 1 },
    { id: 20, category: "스타일링",    name: "헤어 왁스",            par: 4,  current: 3 },
    { id: 21, category: "스타일링",    name: "하드 스프레이",         par: 4,  current: 2 },
    { id: 22, category: "판매용",      name: "홈케어 샴푸",          par: 10, current: 7 },
    { id: 23, category: "판매용",      name: "홈케어 트리트먼트",      par: 10, current: 3 },
  ]);
  ...
  }, [members, log, zoneAssign, zoneDone, rotationDate, thanks, events, birthdays, moods, orders, suggestions, timeBank, reviewDone, instaUploads, personalPurchases, importantMemos, luckyPrizes, luckyTickets, luckyClaimed, luckyWins, inventory, isLoaded]);
                        ...
                        {isMe && myCardExpanded ? (
                          <>
                              ...
                              <div style={{ display: "flex", gap: 5 }}>
                                {Object.entries(STATUS).map(([key, s]) => (
                                  <button key={key} onClick={() => {
                                    setMyStatus(key);
                                    showToast(`${s.emoji} ${s.label}로 변경됐어요`);
                                  }} style={{
                                    flex: 1, background: myStatus === key ? s.color : "#fff",
                                    color: myStatus === key ? "#fff" : C.muted,
                                    borderRadius: 10, padding: "8px 0",
                                    fontSize: 10, fontWeight: 700,
                                    border: `1.5px solid ${myStatus === key ? s.color : C.line}`,
                            ...
                            <div style={{
                              ...
                              {/* 몸 상태 */}
                              <div style={{ fontSize: 9, color: C.muted, marginBottom: 3 }}>🫀 몸 상태</div>
                              ...
                              {/* 마음 상태 */}
                              <div style={{ fontSize: 9, color: C.muted, marginBottom: 3 }}>🌷 마음 상태</div>
                          ...
                          /* 다른 멤버 컨디션 보기만 (+ 접힌 내 카드) */
                          <div
                            onClick={() => { if (isMe) setMyCardExpanded(true); }}
                            style={{
                            ...
                            {memberMood ? (
                              <div style={{
                                ...
                                <span style={{ color: C.muted, fontSize: 10, fontWeight: 600 }}>💗 컨디션</span>
                                <span style={{
                                  display: "inline-flex", alignItems: "center", gap: 3,
                                  background: C.roseSoft, color: C.roseDeep,
                                  borderRadius: 7, padding: "1px 7px",
                                }}>
                                  🫀 <span style={{ fontSize: 13 }}>{bodyEmojis[memberMood.body]}</span>
                                </span>
                                <span style={{
                                  display: "inline-flex", alignItems: "center", gap: 3,
                                  ...
                                  🌷 <span style={{ fontSize: 13 }}>{mindEmojis[memberMood.mind]}</span>
                                </span>
                                {isMe && (
                                  <span style={{ fontSize: 10, color: C.sage, marginLeft: "auto" }}>
                                    ✏️ 수정
                                  </span>
                                )}
                              </div>
                              ...
                              <div style={{
                                fontSize: 10, color: isMe ? C.sage : C.muted, fontStyle: "italic",
                                textAlign: "center", padding: "4px",
                              }}>
                                {isMe ? "✏️ 눌러서 컨디션 체크하기" : "💗 아직 컨디션 체크 전"}
                              </div>
                            )}
                            {memberMood?.note && (
                              <div style={{
                                fontSize: 10, color: C.ink, marginTop: 4,
                                fontStyle: "italic", lineHeight: 1.4,
                                background: C.warm, borderRadius: 6, padding: "4px 8px",
                              }}>
                                "{memberMood.note}"
                              </div>
                            )}
          ...
          <div>
            ...
            {isJangnim ? (
              <div>
                ...
                {(() => {
                  const todayStr = new Date().toLocaleDateString("ko-KR");
                  const myMoodToday = moods.find(x => x.memberId === myId && x.date === todayStr);
                  const bodyEmojis = ["", "😩", "😟", "😐", "🙂", "💪"];
                  const mindEmojis = ["", "😢", "😔", "😌", "😊", "🥰"];

                  // 접힌 상태 (이미 체크함)
                  if (myMoodToday && !myCardExpanded) {
                    return (
                      <div
                        onClick={() => setMyCardExpanded(true)}
                        className="card" style={{
                          background: C.warm, borderRadius: 16,
                          border: `1.5px solid ${C.rose}`, padding: "14px 16px",
                          marginBottom: 16, cursor: "pointer",
                        }}>
                        <div style={{
                          display: "flex", alignItems: "center", gap: 8, fontSize: 12,
                        }}>
                          <span style={{ fontWeight: 700, color: C.ink }}>💗 오늘 내 컨디션</span>
                          <span style={{
                            display: "inline-flex", alignItems: "center", gap: 3,
                            background: C.roseSoft, color: C.roseDeep,
                            borderRadius: 7, padding: "2px 8px",
                          }}>
                            🫀 <span style={{ fontSize: 14 }}>{bodyEmojis[myMoodToday.body]}</span>
                          </span>
                          <span style={{
                            display: "inline-flex", alignItems: "center", gap: 3,
                            background: C.sageSoft, color: C.sageDeep,
                            ...
                            🌷 <span style={{ fontSize: 14 }}>{mindEmojis[myMoodToday.mind]}</span>
                          </span>
                          <span style={{ fontSize: 10, color: C.sage, marginLeft: "auto" }}>
                            ✏️ 수정
                          </span>
                  ...
                  return (
                    <div className="card" style={{
                      ...
                      <div style={{ fontSize: 13, fontWeight: 700, color: C.ink, marginBottom: 10 }}>
                        💗 오늘 내 컨디션은?
                        {myMoodToday && (
                          <span style={{ fontSize: 10, color: C.sageDeep, marginLeft: 6 }}>✓ 체크됨</span>
                        )}
                      </div>

                      {/* 몸 상태 */}
                      <div style={{ fontSize: 10, color: C.muted, marginBottom: 4 }}>🫀 몸 상태</div>
                      ...
                      {/* 마음 상태 */}
                      <div style={{ fontSize: 10, color: C.muted, marginBottom: 4 }}>🌷 마음 상태</div>
          ...
          <div>
            ...
            {salonSub === "stock" && (() => {
              ...
              const addToOrder = (it) => {
                ...
                setOrders(p => [...p, {
                  item: it.name, qty: `${need}개`, urgency: "보통", note: "재고 부족 자동 요청",
                  id: Date.now(), from: myId, status: "요청됨",
                  time: new Date().toLocaleString("ko-KR", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" }),
                }]);
                showToast(`${it.name} 발주 요청에 추가됨 📦`);
              };
              // 같은 분류 안에서 위/아래로 순서 이동
              const moveItem = (id, dir) => setInventory(p => {
                ...
                let j = dir < 0 ? i - 1 : i + 1;
                while (j >= 0 && j < arr.length && arr[j].category !== cat) j += dir;
              ...
              return (
                <div>
                  ...
                  {orders.length > 0 && (
                    <div className="card" style={{
                      ...
                      <div style={{ fontSize: 13, fontWeight: 700, color: C.ink, marginBottom: 8 }}>
                        📦 요청한 발주 <span style={{ color: "#4E8AAD" }}>{orders.length}</span>
                        <span style={{ fontSize: 11, fontWeight: 500, color: C.muted, marginLeft: 6 }}>· 상태를 눌러 바꿀 수 있어요</span>
                      </div>
                      {[...orders].reverse().map(o => {
                        const fromMember = ALL_MEMBERS.find(m => m.id === o.from);
                        const statusColor = o.status === "완료" ? C.sage : o.status === "주문중" ? C.gold : C.muted;
                          ...
                          <div key={o.id} style={{
                            ...
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
                              <div style={{ fontSize: 12, fontWeight: 700, color: C.ink }}>
                                {o.item}{o.qty && <span style={{ color: C.muted, fontWeight: 500 }}> · {o.qty}</span>}
                              </div>
                              <div style={{ fontSize: 10, color: C.muted }}>{fromMember?.name} · {o.time}</div>
                            </div>
                  ...
                  {/* 분류별 목록 */}
                  {INV_CATS.filter(cat => shown.some(it => it.category === cat)).map(cat => (
                      ...
                      {shown.filter(it => it.category === cat).map(it => {
                        ...
                        return (
                          <div key={it.id} className="card" style={{
                            background: C.card, borderRadius: 14, padding: "11px 13px", marginBottom: 7,
                            border: `1px solid ${low ? C.roseSoft : C.line}`,
                            borderLeft: `4px solid ${low ? C.roseDeep : C.sage}`,
                          }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                              <div style={{ fontSize: 13, fontWeight: 700, color: C.ink, flex: 1 }}>{it.name}</div>
                              {low && <span style={{
                                fontSize: 10, padding: "2px 8px", borderRadius: 8,
                                background: C.roseSoft, color: C.roseDeep, fontWeight: 700, marginRight: 6,
                              }}>부족 {need}</span>}
                              {!invShortageOnly && (
                                <>
          ...
          <div>
            ...
            <div className="card" style={{
              ...
              <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.8 }}>
                ...
                시술 서브 요청은 <b style={{ color: C.sageDeep }}>서희·민경·지유·해수</b>님께 부탁해요 🙏<br/>
                <br/>
