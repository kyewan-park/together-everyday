      ...
      <div style={{ flex: 1, overflow: "auto", padding: 16, paddingBottom: 32 }}>
        ...
        {tab === "home" && (
          <div>
            ...
            {/* 🌅 오늘의 빠른 체크 (매일 출근 후 체크) */}
            {!isJangnim && (() => {
              const todayDateStr = new Date().toLocaleDateString("ko-KR");
              const myMoodToday = moods.find(x => x.memberId === myId && x.date === todayDateStr);
              return (
                <div className="card" style={{
                ...
                {members.filter(m => m.canHelp).map(m => {
                  ...
                  return (
                    <div key={m.id} style={{
                      ...
                      border: `1.5px solid ${isMe ? st.color : "transparent"}`,
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{
                          ...
                          {m.name}
                          {isMe && <span style={{ fontSize: 9, color: C.sage, marginLeft: 3 }}>(나)</span>}
                        </div>
                        <div style={{
                          fontSize: 10, color: st.color, fontWeight: 600, marginTop: 2,
                        }}>
                          {st.emoji} {st.label}
                        </div>
                      </div>
